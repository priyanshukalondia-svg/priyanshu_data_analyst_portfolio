...
function Nav() {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? "py-4 bg-cream/85 backdrop-blur-sm" : "py-8"}`}>
      <div className="relative mx-auto flex max-w-[1600px] items-start justify-between px-6 md:px-12">
        ...
        <nav className="hidden md:flex absolute left-1/2 -translate-x-1/2 top-1/2 -translate-y-1/2 items-center gap-10 lg:gap-14">
          {NAV.map((n) => (
            <a
              key={n.id}
              href={`#${n.id}`}
